...
- **สถานะ:** ✅ เฟส 0, 1, **2 (เทียบพอร์ต + เงินเข้าออก)**, **3 (ตลาดไทย)** และ **4 (risk analytics เชิงลึก)** เสร็จแล้ว (ใช้งานจริงที่ folio-lab-gamma.vercel.app)
...
**สถานะจริงเมื่อจบ S16:** ✅ **ปิดเฟสครบทั้ง 2.1–2.6** — US-16 ถึง US-22 เสร็จทั้งเจ็ดใบ
และขึ้น production แล้ว · เกณฑ์ความสำเร็จผ่านทั้งสองข้อ: ลิงก์ที่ยาวที่สุดของเฟส (3 พอร์ตชื่อไทย ·
เกณฑ์เบี่ยงเบน · เงินเข้าออก) เปิดซ้ำได้ผลเหมือนเดิมทุกหลัก · ผล DCA และการปรับสมดุลตรวจทานกับ
การคำนวณมือได้หลายกรณี (`331` / `10.00%` ต่อเดือน · `300` / `0.00%` · หน้าต่าง 1 ปีของชุดคงที่ `12.68%`)
· **พอร์ตอ้างอิงยังได้ `$41,495` / `10.31%` / `−23.55%` เท่าเดิมทุกหลักตลอดทั้งเฟส**

**บันทึกของเฟส:** spec ครบทั้งเฟสที่ S14 — epic `compare-cashflows` + US-16 ถึง US-22 เป็น 🟢 Ready
...
· ลงมือที่ S15 (2.1) · S15b (2.2–2.3) · S16 (2.4–2.6) · เพิ่ม [PD-016](../docs/product/decision-log.md) ตอน S15b (วิธีกระจายเงินที่ใส่เพิ่มเป็นตัวเลือกของผู้ใช้)
...
**สถานะเมื่อจบ S17 (spec พร้อม ยังไม่ลงมือ):** epic [`risk-analytics`](product/stories/risk-analytics/epic.md)
เขียนครบ — US-28..US-33 อยู่ที่ 🟢 Ready ทั้งหกใบ · ทุกเมทริกมีสูตร **หน่วยเวลา** และค่าคาดหวัง
พร้อมที่มา ([PD-019](product/decision-log.md)) · เมทริกรายสินทรัพย์แยกเป็นฟังก์ชันใหม่ ไม่แตะลูป
ที่ golden ยึดไว้ ([PD-020](product/decision-log.md)) · ทะเบียนศัพท์เติมครบ 20 แถวของเฟสนี้แล้ว

สองข้อที่ปรับจากแผนเดิมของเฟส ตอนเขียนการ์ด:
- **4.4** ต้มยำกุ้ง 2540 อยู่นอกช่วงข้อมูลที่มี (เริ่ม 2012) → US-32 ใช้สี่ช่วงที่ข้อมูลครอบคลุมจริง
  (โควิด 2563 · เงินเฟ้อ 2565 · เทขายปลายปี 2561 · จีนและน้ำมัน 2558–2559) และบอกผู้ใช้ว่า
  ดูย้อนได้ถึงปีไหน แทนที่จะแสดงช่วงที่คำนวณไม่ได้
- **4.5** อัตราถอนคิดเป็นเปอร์เซ็นต์ของ**เงินตั้งต้นที่ปรับตามเงินเฟ้อ** และต้องรอด**ทุกหน้าต่าง**
  ของอดีต ไม่ใช่หน้าต่างเฉลี่ย (US-33)

วิธีคิด VaR ถูกยืนยันกับค่าอ้างอิงในภาคผนวก A แล้วตอนเขียนการ์ด — มีวิธีเดียวที่ตรงในแต่ละค่า
ซึ่งเอาความกำกวมที่ใหญ่ที่สุดของเฟสนี้ออกไปก่อนลงมือ (ดู US-31)

**สถานะเมื่อจบ S18 — 4.1 · 4.2 · 4.3 ปิดแล้ว:** US-28..US-31 ✅ Done ทั้งสี่ใบและอยู่บน production
· ตารางสรุปโตจาก 9 เป็น 22 แถว แบ่งสองกลุ่มพร้อมหัวกลุ่มคั่น · เพิ่มสองส่วนใหม่คือ
ตารางความสัมพันธ์และตารางแยกส่วนความเสี่ยง ซึ่งมีตัวเลือกพอร์ตเมื่อเทียบหลายพอร์ต

ค่าที่เทียบกับภาคผนวก A ได้ **ตรงครบทุกตัว**: Beta `0.79` · Alpha `−1.36%` · R² `93.14%`
· upside/downside capture `72.18%`/`85.25%` · VaR `5.24%` · VaR เชิงสถิติ `4.55%` · CVaR `7.04%`
· correlation VTI–VXUS `0.83` · VTI–BND `0.36`

**สองสูตรในสเปกถูกจับได้ว่าผิดด้วยค่าอ้างอิงพวกนี้เอง** ([PD-024](product/decision-log.md)):
Alpha ต้องคิดจากผลตอบแทนดิบไม่ใช่ส่วนเกิน · capture ต้องแปลงผลทบต้นเป็นรายปีก่อนหาร
· ข้อสังเกตที่สำคัญกว่าตัวแก้คือ **Beta กับ R² ตรงทั้งสองนิยาม มีแต่ Alpha ที่แยกออกจากกันได้**
— ถ้าไม่ได้บังคับให้มีค่าอ้างอิงของ Alpha ไว้ด้วย เราจะ ship สูตรที่ผิดโดยที่อีกสองค่า "ผ่าน" อยู่

**สถานะเมื่อจบ S19 — 🎯 ปิดเฟส 4 ครบ 4.1–4.5:** US-32 (ช่วงวิกฤต) และ US-33 (อัตราถอนปลอดภัย)
✅ Done และอยู่บน production · ตารางช่วงวิกฤตสี่ช่วงพร้อมคำอธิบายว่าเกิดอะไรขึ้น และตารางอัตราถอน
ที่บอกทั้งเปอร์เซ็นต์และจำนวนเงินต่อเดือน

**ค่าอ้างอิงของช่วงวิกฤต:** เงินเฟ้อ 2022 `−23.55%` และจีน/น้ำมัน 2015–16 `−8.48%` ตรงกับ
ตารางช่วงขาดทุนที่ ship ไปแล้ว เพราะเป็นเดือนชุดเดียวกันพอดี · อีกสองช่วงหน้าต่างคลาดกันหนึ่งเดือน
โดยตั้งใจ จึงไม่ใช่ค่าอ้างอิง ([PD-025](product/decision-log.md))

**อัตราถอนของพอร์ตอ้างอิง:** ระยะ 10 ปี `13.15%` ต่อปี (`$110`/เดือนจากเงินตั้งต้น `$10,000`)
ทดสอบจาก 55 จังหวะเข้า หน้าต่างที่บีบที่สุดเริ่ม ก.ย. 2014 · ระยะ 20 และ 30 ปีไม่มีค่า
เพราะข้อมูล 174 เดือนสั้นกว่าระยะนั้น

**สิ่งที่ตัดออกอย่างตั้งใจตามข้อ 6 ยังไม่ทำ:** Monte Carlo · factor regression · efficient frontier

**ถัดไป:** เฟส 5 — AI layer + case study
...
### งานข้ามเฟส — แคตตาล็อกสินทรัพย์ (S16b) ✅

ไม่ได้อยู่ในแผนเดิม · เกิดขึ้นหลังปิดเฟส 2 เพราะเห็นชัดว่าความสามารถที่สร้างมาสามเฟส
(เทียบ 3 พอร์ต · เงินเข้าออก · ปรับสมดุล · หน้าต่างเลื่อน) ถูกขังไว้ด้วยรายการสินทรัพย์ 10 ตัว
ที่เลือกมาเพื่อ**พิสูจน์ระบบ**ตั้งแต่ S2/S11 ไม่ใช่เพื่อให้คนใช้เลือก

ผลลัพธ์: แคตตาล็อก **100 ตัว (ไทย 50 · ต่างประเทศ 50)** ใน 19 หมวด ทุกตัวยิงถามแหล่งข้อมูลผ่าน ·
กล่องเลือกแบ่งหมวดพร้อมหัวข้อ · สินทรัพย์ที่ข้อมูลย้อนหลังสั้นบอกปีที่ข้อมูลเริ่มก่อนผู้ใช้เลือก
([PD-017](../docs/product/decision-log.md)) · มี `scripts/probe-symbols.mjs` ไว้ตรวจว่าแคตตาล็อก
ยังตรงกับความจริง เพราะตั๋วตายและเปลี่ยนชื่อได้ (เจอจริงระหว่างทำ: MAKRO · ESSO · BANPU)
