...
test.describe("US-20 ผลตอบแทนแบบหน้าต่างเลื่อน", () => {
  ...
  test("AC-CMP-70 ค่าไม่เปลี่ยนเมื่อเปิดเงินเข้าออกและปรับเงินเฟ้อ พร้อมข้อความกำกับ", async ({ page }) => {
    ...
    await expect(
      // ประโยคนี้ใช้ทั้งที่ส่วนหน้าต่างเลื่อนและที่ส่วนความสัมพันธ์ตั้งแต่เฟส 4
      // จึงผูกกับตัวชี้ของส่วนนี้โดยตรง แทนการค้นด้วยข้อความที่ไม่ซ้ำอีกต่อไป
      page.getByTestId("rolling-pure-note"),
    ).toBeVisible()
  })
